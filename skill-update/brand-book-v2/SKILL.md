...
name: brand-book-v2
description: >
  VERSION EXPÉRIMENTALE CRÉATIVE du pipeline brand book. Même base que
  brand-book (interview, brand.md, landing, slots masque, presets) mais
  ajoute une couche de composition pilotée par l'identité : GESTE
  signature récurrent dérivé de l'univers, ARCHÉTYPES de mise en page
  (Éditorial magazine, Grille suisse, Poster brutaliste, Organique) en
  plus des presets couleur, et checkpoint d'exploration. Objectif : que
  deux marques aient des mises en page différentes DANS LEURS OS, pas
  juste leur couleur. NE DÉCLENCHER QUE sur demande explicite de la
  version créative/expérimentale : "brand book v2", "version créative du
  brand book", "brand book créatif", "teste la v2", "geste signature",
  "archétype de mise en page", "brand book plus créatif". Pour le pipeline
  stable de production, utiliser le skill "brand-book" (sans v2).
---

# Skill : brand-book-v2 (créatif / expérimental)

> **Différence avec `brand-book` (v1 stable)** : la v1 a un squelette de
> mise en page fixe + 5 presets qui ne changent que la peau (couleur,
> typo, traitement image). La v2 ajoute **3 leviers de créativité pilotés
> par l'identité**, documentés dans les sections « ✦ V2 » de ce fichier :
> 1. **Geste signature** (Étape 4-GESTE) : un motif graphique unique
>    dérivé de Q12+Q14, filé sur les 15 slides.
> 2. **Archétypes de composition** (Étape 4-ARCHÉTYPE) : la grammaire de
>    placement (grille, marges, hero, bleed) varie selon l'identité, en
>    plus du preset couleur.
> 3. **Checkpoint d'exploration** (Étape 0-EXPLO) : 2-3 directions de
>    cover en HTML, Arnaud pilote avant de bâtir les 15 slides.
>
> Tout le reste (interview, brand.md, landing, slots masque, prompts
> images) est identique à la v1 et reste valable tel quel ci-dessous.

---

## 🚦 RÈGLE D'EXÉCUTION N°1 — L'INTERVIEW AVANT TOUT (lire en premier)

En mode CRÉATION, cet ordre est **non négociable** et ne peut pas être
abrégé :

1. **POSER** les 5 vagues d'interview (Étape A.2) via `AskUserQuestion`,
   une vague à la fois, et **ATTENDRE** les réponses de l'utilisateur.
2. **SEULEMENT APRÈS**, écrire `brand.md`.
3. Choisir preset (4bis) + geste (4-GESTE) + archétype (4-ARCHÉTYPE).
4. Annoncer la durée (cf. séquence), puis écrire `images-brief.md`.
5. GATE images → génération Figma.

**INTERDITS FORMELS** :
- ❌ Écrire `brand.md`, `landing-page.md` ou `images-brief.md` **avant**
  d'avoir appelé `AskUserQuestion` et reçu les réponses. Si tu t'apprêtes
  à écrire l'un de ces fichiers sans avoir posé les questions → **STOP**,
  reviens à la Vague 1.
- ❌ **INVENTER ou DÉDUIRE** les réponses à la place de l'utilisateur. Le
  `brand.md` doit refléter SES réponses, pas tes hypothèses. Un `brand.md`
  écrit sans questions est un livrable faux.
- ❌ Sauter l'interview parce qu'un `brand.md` existe déjà dans le dossier.

**Si un `brand.md` existe déjà au lancement** : NE PAS le réutiliser en
silence. Poser d'abord **une** question :
> « J'ai trouvé un `brand.md` existant pour [nom]. Tu veux (1) une
> **nouvelle charte** (je repose l'interview complète et j'écris un
> nouveau brand.md) ou (2) **régénérer le book** depuis ce brand.md
> existant ? »

et n'enchaîner qu'après la réponse. En cas de doute, considérer que
lancer « brand book v2 » = nouvelle charte = CRÉATION = interview complète.

> **Pourquoi ce garde-fou** : en test (2026-06), le skill a sauté
> l'interview et écrit `brand.md` directement — soit en inventant les
> réponses, soit parce qu'un `brand.md` traînait dans le dossier. Les
> deux cas produisent une charte qui ne reflète pas la marque.
> **L'interview EST le produit.** Sans elle, tout le reste (preset, geste,
> archétype, prompts) est calibré sur du vent.

---

## ✦ V2 · Workflow IMAGES-FIRST (mode par défaut)

**Pourquoi** : l'agent sait **cloner** une image déjà dans le fichier
Figma, mais pas en importer une (upload/createImage bloqués). Donc au lieu
de générer le book avec des placeholders puis remplir après, on **fait
entrer les images d'abord**, et le book sort habillé du premier coup.

> ⛔ **CE MODE NE RACCOURCIT PAS L'INTERVIEW.** Il change seulement
> l'ORDRE relatif (prompts AVANT la génération Figma), pas le fait que
> `brand.md` doit être **complet** d'abord. Les 17 prompts sont
> **dérivés** de l'univers (Q11-Q14), de la palette, du preset, du geste
> et de l'archétype : sans ces réponses, ils sont génériques et inutiles.
> Si tu te retrouves à écrire `images-brief.md` après seulement 2-3
> questions, c'est un BUG : remonte faire l'interview.

### ✅ Pré-requis AVANT d'écrire `images-brief.md` (bloquant)

N'écrire les prompts QUE si **tout** ce qui suit est fait. Checklist à
vérifier explicitement :

- [ ] **Interview complète** : les 5 vagues / 18 questions de l'Étape A.2
  ont été posées (positionnement, ton, identité verbale, typo+couleurs,
  univers, persona). Pas 2, pas 3 : les 5 vagues.
- [ ] **`brand.md` écrit et complet** (toutes les tables : Couleurs,
  Typographie, Univers visuel Q11-Q14).
- [ ] **Preset** choisi (Étape 4bis).
- [ ] **Geste signature** choisi (Étape 4-GESTE).
- [ ] **Archétype** choisi (Étape 4-ARCHÉTYPE) — il change le ratio/cadrage
  attendu de certaines images (ex : `poster` → cover en fullbleed).

Tant qu'une case n'est pas cochée → **ne pas générer les prompts**,
continuer l'interview. C'est le défaut le plus fréquent de ce mode :
l'agent fonce vers les prompts et saute l'identité.

### Séquence

1. **Phase A complète** : interview 5 vagues (A.2) + vocabulaire vivant
   (A.2.bis) → extraction tokens (A.3) → `brand.md` (A.4) → **preset
   (4bis) + geste (4-GESTE) + archétype (4-ARCHÉTYPE)**. C'est le gros de
   la conversation : une douzaine de questions minimum, pas deux.
2. **Annonce + choix AVANT de rédiger les prompts (obligatoire)** : c'est
   l'étape la plus longue (chaque prompt est rédigé sur-mesure), donc on
   prévient au lieu de lancer en silence. Poser, via `AskUserQuestion` :
   > « Prochaine étape : je rédige les prompts images sur-mesure
   > (l'étape la plus longue de la charte, ~1 à 2 min de génération).
   > Tu veux : les **17** (deck 100% photo) · les **10 prioritaires**
   > (cover, identité, univers, chapitres, hiérarchie, éléments) ·
   > ou **plus tard** (placeholders dégradés pour l'instant) ? »
   Défaut si l'utilisateur ne répond pas : 10 prioritaires. Ne jamais
   écrire les 17 sans avoir prévenu de la durée.
3. Écrire `images-brief.md` **numéroté dans l'ordre de consommation**
   (1→17, ou 1→10 selon le choix), avec par image : prompt + ratio +
   boîte pixels (cf. Étape 9ter v2). Les prompts reprennent l'univers, la
   palette hex et le traitement Q13 de `brand.md`, et le cadrage dicté
   par l'archétype.
4. **GATE (bloquant)** : le skill s'arrête et demande à Arnaud de
   préparer les images dans Figma (générées via son plugin IA, ou ses
   propres photos), regroupées dans **un seul groupe**, puis de **coller
   l'URL du groupe**. Message type :
   > « `brand.md` et les prompts sont prêts. Génère/dépose tes images
   > dans un groupe Figma (dans l'ordre du fichier de prompts, lecture
   > gauche→droite, haut→bas), puis colle-moi l'URL du groupe. Tu veux
   > lancer le book maintenant, ou d'abord préparer les images ? »
5. Arnaud fournit l'URL du groupe (ou dit « lance avec placeholders »).
6. Génération Figma : je lis les enfants du groupe, je les mappe aux
   slots dans l'ordre, je les clone dans les masques, j'habille les 15
   slides (preset × geste × archétype).
7. **Vérif** : screenshot des slides + récap du mapping ; Arnaud peut
   demander des swaps (« inverse 3 et 4 ») sans tout relancer.

### Ordre canonique des 17 slots (index de mapping)

C'est l'ordre dans lequel je consomme le groupe **et** la numérotation de
`images-brief.md`. Fixe, jamais réordonné :

```
1 cover · 2 identite · 3 univers-1 · 4 univers-2 · 5 univers-3 ·
6 univers-4 · 7 univers-5 · 8 univers-6 · 9 chapitre-logo ·
10 logo-hero · 11 chapitre-couleur · 12 nuances · 13 chapitre-typo ·
14 police · 15 hierarchie · 16 chapitre-systeme · 17 elements
```

(Les slots `sommaire-*` et `palette-col-*` restent décoratifs : ils
gardent leur placeholder dégradé, hors des 17.)

### Consommation du groupe « en vrac » (par ordre de lecture)

```js
// groupNode = node fourni par l'URL d'Arnaud (getNodeByIdAsync)
const SLOT_ORDER = ["cover","identite","univers-1","univers-2","univers-3",
 "univers-4","univers-5","univers-6","chapitre-logo","logo-hero",
 "chapitre-couleur","nuances","chapitre-typo","police","hierarchie",
 "chapitre-systeme","elements"];

function readPhotoGroup(groupNode) {
  const kids = groupNode.children.filter(n => n.visible !== false);
  // ordre de lecture : haut→bas par bandes de ~200px, puis gauche→droite
  kids.sort((a,b) => {
    const ra = Math.round(a.y/200), rb = Math.round(b.y/200);
    return ra !== rb ? ra - rb : a.x - b.x;
  });
  const map = {};                       // slot -> node photo
  SLOT_ORDER.forEach((slot,i) => { if (kids[i]) map[slot] = kids[i]; });
  return map;                           // alimente IMG_TPLS pour placeImage
}
```

`readPhotoGroup()` **alimente `IMG_TPLS`** (la map que `placeImage` lit
déjà). Donc rien d'autre à changer dans les slides : `placeImage` clone
la photo mappée dans le masque du slot, exactement comme un node
`IMG · slot`.

### Précédence des sources d'images

1. Node **`IMG · <slot>`** nommé sur Page 1 (contrat explicite v1) — gagne
   toujours s'il existe.
2. Sinon, **groupe en vrac** mappé par ordre de lecture (ce workflow).
3. Sinon, **placeholder dégradé** du preset + `images-brief.md`.

> **De-risk de l'ordre** : comme le mapping « en vrac » dépend de la
> disposition, la **vérif de l'étape 6 est obligatoire** : présenter le
> mapping (« slot 3 univers-1 ← 3ᵉ photo du groupe ») et un screenshot,
> et permettre des swaps rapides. Si Arnaud préfère le zéro-risque, il
> peut nommer ses calques `IMG · <slot>` (précédence #1).
...
| `brand.md` présent + nouvelle URL Figma vierge | **BOOK** | Phase B uniquement (brochure depuis brand.md existant) — ⚠️ **uniquement après confirmation explicite** (cf. RÈGLE N°1 : un brand.md qui traîne ne déclenche jamais BOOK en silence ; demander d'abord nouvelle charte vs régénération) |
...
## ✦ V2 · Étape 4-GESTE — Le geste signature (levier créatif #1)

**Le problème que ça résout** : sans geste, deux brand books ne diffèrent
que par la couleur. Le geste signature est **un seul motif graphique**
dérivé de l'identité, filé sur les 15 slides comme du mobilier de page,
et amplifié en « moment héros » sur la cover et les chapter openers.
C'est ce qui fait dire « ce livre a été pensé pour CETTE marque ».

### Catalogue des gestes (motif ← identité)

| Geste | Idée | Tracé | Identité (Q12 univers / Q14 perso / Q1 ton) |
|---|---|---|---|
| `piste` | Progression, mouvement | Ligne fine traversante + point plein qui **avance selon le n° de slide** (slide 1 = départ, slide 15 = arrivée) | course, sport, mouvement, data temporelle ; perso Énergique ; promesse « performance » |
| `colonnes` | Structure, institution | 3-5 filets verticaux pleine hauteur, opacité faible | architecture, droit, finance, corporate ; perso Sobre/Premium ; ton Confidentiel |
| `arc` | Souplesse, soin, luxe | Grand arc de cercle ancré dans un angle, stroke fin | nature, éditorial luxe, bien-être ; perso Chaleureux/Émotionnel |
| `module` | Système, précision | Grille de points + ticks de repère dans un angle | tech minimale, data, SaaS ; perso Technique |
| `oblique` | Franchise, impact | Barre oblique épaisse ou coupe diagonale d'angle | ton Direct ; perso Audacieux ; mode, événement |
| `cercle` | Communauté, focus | Anneau + crops ronds récurrents | humain, communauté, coaching ; ton Chaleureux |
| `encoche` | Retail, étiquette | Coins entaillés (ticket/tag) sur cards et slots | retail, mode, food, événementiel |

### Sélection (vote, comme les presets)

Compter les voix Q12 + Q14 + Q1 (+ promesse Q4). Le geste qui domine
gagne. **Départage par la géométrie du logo** (lue via `get_screenshot` /
ratio du node) : logo italique ou dynamique → `piste`/`oblique` ; mark
rond → `cercle`/`arc` ; logo très vertical → `colonnes`. Annoncer le
geste retenu en 1 ligne dans le récap. En cas d'égalité franche, une
seule `AskUserQuestion` avec les 2-3 gestes favoris.

Écrire le geste choisi dans `brand.md` (section Univers visuel →
`**Geste signature**`) pour que le mode BOOK le retrouve.

### Threading — 3 niveaux d'intensité

1. **Mobilier (les 15 slides)** : le geste à faible poids (opacité 8-15%,
   ou filet d'accent 1-2px), toujours au même endroit (un bord, un angle),
   **jamais en collision avec le contenu**. Posé en premier, sous tout.
2. **Numéro incarné** : quand c'est naturel, le geste **porte** le numéro
   de page ou de chapitre (le point de `piste`, un tick de `module`, une
   `encoche`). Le footer devient signature au lieu d'être neutre.
3. **Héros (cover + 4 chapter openers)** : le geste **à pleine échelle**
   comme élément graphique majeur — la ligne `piste` traverse toute la
   cover, l'`arc` encadre le titre, la grille `module` remplit un quadrant.

### Helper `drawGesture()` (à coder dans le préambule Étape 5)

```js
// GESTE = { id: "piste"|"colonnes"|"arc"|... }  (choisi ci-dessus)
// role : "furniture" (défaut, discret) | "hero" (pleine échelle)
function drawGesture(slide, { index, total, role = "furniture" }) {
  const g = GESTE.id, A = COLORS.accent, prog = (index - 1) / (total - 1);
  if (g === "piste") {
    const y = role === "hero" ? 1180 : 1248;            // bas de slide
    const ln = figma.createRectangle();
    ln.resize(1920 - 320, role === "hero" ? 3 : 1.5);
    ln.x = 160; ln.y = y; ln.fills = solidVar("accent");
    ln.opacity = role === "hero" ? 1 : 0.5; slide.appendChild(ln);
    const dot = figma.createEllipse();
    const d = role === "hero" ? 28 : 12;
    dot.resize(d, d); dot.fills = solidVar("accent");
    dot.x = 160 + prog * (1920 - 320) - d / 2; dot.y = y + (role === "hero" ? 3 : 1.5) / 2 - d / 2;
    slide.appendChild(dot);
  }
  // … un bloc par geste (colonnes, arc, module, oblique, cercle, encoche)
  // Chaque bloc respecte : posé sous le contenu, opacité faible en
  // furniture, jamais sur la safe zone de texte.
}
```

**Appel** : dans `makeSlide()` ou juste après, appeler
`drawGesture(slide, { index, total })` pour les 15 slides (furniture), et
`drawGesture(slide, { index, total, role: "hero" })` en plus sur 01 et les
chapter openers. Le geste se pose **avant** les slots images et les
textes (z-order : geste furniture tout au fond).

> **Garde-fou** : le geste est un **accent**, pas un tapis. En furniture
> il ne doit jamais dépasser ~15% d'opacité ni croiser un bloc de texte.
> Si la QA 9bis voit le geste gêner la lecture, le repousser vers le bord
> ou baisser l'opacité — ne jamais le supprimer (il porte l'identité).

---

## ✦ V2 · Étape 4-ARCHÉTYPE — La grammaire de composition (levier créatif #2)

**Le vrai fix structurel.** Le preset change la peau, le geste signe ;
l'archétype change **les os** : grille, marges, taille et place du héros,
alignement, densité, comportement des images. C'est lui qui fait que deux
marques ont des mises en page réellement différentes, pas juste des
couleurs différentes.

> **Principe d'implémentation** : en v1, l'Étape 8 fixe des x/y absolus.
> En v2, ces positions deviennent **dérivées de l'archétype**. Chaque
> slide lit un objet `ARCHETYPE` (grille + règles) et calcule ses
> positions, au lieu de coordonnées en dur. L'Étape 8 reste la référence
> des *contenus* par slide ; l'archétype décide *où et comment* ils se
> posent.

### Les 4 archétypes

| Archétype | Grille / marges | Héros (cover, openers) | Images | Densité | Identité |
|---|---|---|---|---|---|
| **Éditorial magazine** | Asymétrique, marges larges (120-160), 1 colonne dominante + 1 étroite | Type grand, calé en bas-gauche, ferré à gauche en drapeau | Bleeds latéraux assumés | Aérée | luxe, mode, éditorial ; Q14 Premium/Émotionnel |
| **Grille suisse** | Modulaire stricte 12 col, marges égales (80), gouttières visibles | Type mesuré **aligné sur les modules**, métadonnées ferrées en haut-droite | Cadrées dans la grille, jamais de bleed | Dense, numérotée | tech, corporate, systématique ; Q14 Technique/Sobre |
| **Poster brutaliste** | Pas de grille, le type EST la grille | Type **géant en capitales touchant les bords**, peu d'éléments | Blocs full-bleed, chevauchements | Minimale, choc | pop, sport, événement ; Q1 Direct, Q14 Audacieux |
| **Organique** | Axial centré, hors-grille douce, beaucoup d'air | Type centré, composition symétrique respirante | Crops ronds, formes douces | Très aérée | nature, soin, communauté ; Q1 Chaleureux |

### Objet `ARCHETYPE` (à brancher dans le préambule Étape 5)

```js
const ARCHETYPES = {
  editorial: { margin:{t:120,l:140,r:140,b:132}, cols:1, align:"left",
    coverTitle:{size:200, x:140, baselineY:920, align:"LEFT", case:"none"},
    openerWord:{align:"LEFT", pos:"bottom-left", max:0.6},
    imageMode:"bleed", whitespace:"generous" },
  suisse: { margin:{t:80,l:80,r:80,b:132}, cols:12, gutter:24, align:"modular",
    coverTitle:{size:120, x:80, baselineY:560, align:"LEFT", case:"none", snapGrid:true},
    openerWord:{align:"LEFT", pos:"top-left", max:0.5, showGrid:true},
    imageMode:"contained", whitespace:"dense" },
  poster: { margin:{t:64,l:64,r:64,b:120}, cols:1, align:"bleed",
    coverTitle:{size:420, x:40, baselineY:980, align:"LEFT", case:"upper", edgeBleed:true},
    openerWord:{align:"LEFT", pos:"center", max:0.95, case:"upper"},
    imageMode:"fullbleed", whitespace:"minimal" },
  organique: { margin:{t:120,l:160,r:160,b:132}, cols:1, align:"center",
    coverTitle:{size:160, x:null, baselineY:680, align:"CENTER", case:"none"},
    openerWord:{align:"CENTER", pos:"center", max:0.7, round:true},
    imageMode:"round", whitespace:"generous" },
};
let ARCHETYPE = ARCHETYPES[archId];   // archId choisi par vote ci-dessous
```

### Sélection (vote indépendant du preset)

L'archétype et le preset sont **deux axes séparés** : un même preset
couleur peut tomber sur des archétypes différents. Vote Q1 + Q12 + Q14 +
échelle Q9 :

| Réponse | Vote archétype |
|---|---|
| Q9 Dramatique · Q14 Audacieux · Q1 Direct | poster |
| Q12 Éditorial luxe · Q14 Premium/Émotionnel | editorial |
| Q12 Tech minimale · Q14 Technique/Sobre · Q1 Technique | suisse |
| Q1 Chaleureux · Q12 Nature · Q14 Chaleureux | organique |
| Q9 Contraste fort + corporate | suisse ou editorial (départage logo) |

Départage par la géométrie du logo et l'ambiance. Annoncer l'archétype
retenu (« Archétype : Grille suisse, dérivé de tech + sobre + technique »).
Écrire dans `brand.md` → section Univers visuel → `**Archétype**`. En cas
d'égalité, **une** `AskUserQuestion` avec aperçu 1 ligne de chaque.

> **Pourquoi 2 axes (preset × archétype) et pas un seul** : on multiplie
> la variété sans exploser la complexité. 5 presets × 4 archétypes = 20
> combinaisons de rendu, toutes maîtrisées, à partir d'un seul squelette
> de contenu. Deux marques « Pop contrasté » peuvent diverger fortement
> si l'une est `poster` et l'autre `suisse`.

### Comment les slides consomment l'archétype

Remplacer, dans les specs de l'Étape 8, les constantes de position par
des lectures de `ARCHETYPE` :
- `slide.x0 = ARCHETYPE.margin.l`, largeur utile = `1920 - l - r`.
- Titre cover : `ARCHETYPE.coverTitle.{size,x,baselineY,align,case}`
  (le `case:"upper"` met en capitales pour `poster`).
- Mot d'opener : `ARCHETYPE.openerWord.{align,pos,max}` — `max` = part de
  largeur max occupée par le mot (réduire la police jusqu'à tenir).
- Grille visible (`showGrid`) et snap (`snapGrid`) uniquement pour
  `suisse`.
- `imageMode` pilote `placeImage` : `contained` (cadre net),
  `bleed` (déborde d'un côté, `clipsContent` rogne), `round` (masque
  elliptique au lieu de rectangle), `fullbleed` (voir ci-dessous).

### `fullbleed` — image de fond pleine page (archétype Poster)

**Correctif Stride (2026-06)** : en `poster`, la cover et les chapter
openers ne sont **pas** des aplats de couleur avec un mot posé dessus.
L'image (slot `cover` / `chapitre-*`) passe **en fond pleine page**, le
type géant se pose **par-dessus**, et un voile garantit la lisibilité.
C'est ce qui transforme le poster en « piste » au lieu d'un aplat plat.

Recette `placeImage` en mode `fullbleed` :
1. Slot masque = la slide entière (1920×1280), image clonée en COVER.
2. **Voile** par-dessus l'image : rectangle plein `primary`
   (ou `primaryDark`) à **opacité 0.45-0.55**, pour assombrir et faire
   ressortir le type.
3. Type géant + eyebrow + signature posés **après le voile**, en
   `accent` ou `cream` (jamais une couleur sombre sur photo sombre).
4. Le geste signature passe en `accent` par-dessus le tout.
> Sans image disponible : le fond reste l'aplat du preset + placeholder
> dégradé (le poster fonctionne aussi à vide, juste moins « piste »).

### Application par slide selon l'archétype (relance déterministe)

Table de référence pour qu'une **relance from scratch** applique bien
l'archétype, slide par slide. Les positions de l'Étape 8 sont la **base
`editorial`** ; les autres archétypes les transforment ainsi :

| Slide | editorial (base Étape 8) | suisse | poster | organique |
|---|---|---|---|---|
| 01 Cover | logo h-g, type 200 bas-g, image bleed droite | type 120 calé grille, métadonnées h-d, image contenue, grille visible | **image fullbleed + voile**, STRIDE capitales 400+ centré, type accent | type 160 centré, image en crop rond, axial |
| Chapter openers | mot bas-g + image droite | mot h-g aligné grille + image contenue | **mot capitales géant centré sur image fullbleed + voile** | mot centré + forme ronde |
| Content (03,06,08,09,11,12,14) | colonne g 380 + zone droite | tout calé 12 col, gouttières visibles, dense | 1 idée/slide, type plus gros, marges 64, blocs francs | centré/aéré, marges 160, slots ronds |
| Footer / geste | inchangés (le geste suit son propre helper) | idem + n° de page en mono aligné grille | idem, geste accent épais | idem, geste discret |

**Règle d'or de la relance** : au début de la Phase B, choisir
`archId` (vote 4-ARCHÉTYPE) AVANT de coder les slides, instancier
`ARCHETYPE`, et **lire ses champs au lieu des constantes en dur** à
chaque slide. Si une valeur d'archétype manque pour un cas précis,
retomber sur la base `editorial` (jamais d'erreur bloquante).

**Garde-fous hérités** : la QA 9bis, les guardrails (logo réel, zéro
tiret cadratin, slots masque, anti-fuite) restent valables tels quels.
L'archétype ne touche qu'aux positions/tailles, jamais aux règles de
...
> **✦ V2 — lecture obligatoire avant d'appliquer une position** : les
> coordonnées de cette Étape 8 sont la **base `editorial`**. En v2, avant
> de poser chaque élément, le moduler selon `ARCHETYPE` (table
> « Application par slide » de l'Étape 4-ARCHÉTYPE) : marges, taille et
> place du titre/mot, grille visible, `imageMode` (dont `fullbleed` pour
> le poster). Le preset gère la couleur, le geste se pose via son helper,
> l'archétype gère les os. En cas de valeur d'archétype absente, retomber
> sur la base ci-dessous.

### 01 · Couverture
- Fond : `PRESET.bg.cover` (luxe : `primaryDark` ; crème : `cream` ; etc.)
  En `poster` : pas d'aplat, image `cover` en **fullbleed + voile** (cf. 4-ARCHÉTYPE).
...
## Étape 9ter — Écrire `images-brief.md` (17 prompts numérotés, ordre de consommation)

> ⛔ **Pré-requis bloquant** : ne PAS écrire ce fichier tant que la
> checklist « Pré-requis AVANT d'écrire images-brief.md » (Workflow
> IMAGES-FIRST) n'est pas entièrement cochée — interview 5 vagues +
> brand.md complet + preset + geste + archétype. Sinon les prompts sont
> génériques.

**En v2 (mode images-first)**, ce fichier est généré **avant** la
génération Figma (cf. « Workflow IMAGES-FIRST »), pas à la fin. Il liste
les **17 prompts numérotés 1→17 dans l'ordre exact de consommation du
groupe** (cf. ordre canonique des 17 slots). Arnaud génère/dépose ses 17
images dans un groupe **dans ce même ordre** (lecture gauche→droite,
haut→bas), puis me donne l'URL.

**Intention** : chaque prompt est **autoportant** et exécutable direct
dans le plugin IA Figma : sujet, composition, ratio et direction
artistique (palette hex + traitement) déjà dans le bloc. Le **numéro** et
le **ratio** sont la glue avec le mapping : la photo n°5 du groupe →
slot 5.

**Les 17 prompts (ordre fixe = ordre de consommation)** :

| # | Slot | Slide | Ratio · boîte px indicative |
|---|---|---|---|
| 1 | `cover` | 01 | vertical 2:3 · ~700×1280 |
| 2 | `identite` | 03 | portrait 3:4 · 513×669 |
| 3 | `univers-1` | 04 | paysage 3:2 · 346×232 |
| 4 | `univers-2` | 04 | paysage 3:2 · 346×232 |
| 5 | `univers-3` | 04 | paysage 3:2 · 346×232 |
| 6 | `univers-4` | 04 | paysage 3:2 · 346×232 |
| 7 | `univers-5` | 04 | paysage 3:2 · 346×232 |
| 8 | `univers-6` | 04 | paysage 3:2 · 346×232 |
| 9 | `chapitre-logo` | 05 | carré 1:1 · 696×677 |
| 10 | `logo-hero` | 06 | paysage 3:2 · 1260×844 |
| 11 | `chapitre-couleur` | 07 | vertical 1:3 · 347×1020 |
| 12 | `nuances` | 09 | 4:3 · 320×272 |
| 13 | `chapitre-typo` | 10 | portrait 3:4 · 485×642 |
| 14 | `police` | 11 | paysage · 968×705 |
| 15 | `hierarchie` | 12 | vertical 1:2 · 681×1280 |
| 16 | `chapitre-systeme` | 13 | carré 1:1 · 615×695 |
| 17 | `elements` | 14 | panoramique 6,5:1 · 1668×255 |

Chaque entrée du `.md` = `## N · <slot> · <ratio>` + le prompt FR
autoportant (sujet, composition, DA palette hex + traitement Q13). Les
6 `univers` partagent le même traitement (série cohérente), un sujet
chacun (labels Q12). Rappeler en tête du fichier : « ajoute tes 17
images dans un groupe Figma dans cet ordre, puis donne-moi l'URL du
groupe ».

Les slots `sommaire-*` et `palette-col-*` restent décoratifs
(placeholders dégradés), hors des 17.
